...
func TestCheckoutCreatesOrderPendingPayment(t *testing.T) {
	...
	if err != nil { t.Fatalf("cart store init: %v", err) }
	orderStore, err := NewStore(ctx, db)
	if err != nil { t.Fatalf("orders store init: %v", err) }

	c, err := cartStore.CreateCart(ctx)
	if err != nil { t.Fatalf("create cart: %v", err) }
	var variantID string
	if err := db.QueryRowContext(ctx, "SELECT id FROM product_variants LIMIT 1").Scan(&variantID); err != nil {
		if err == sql.ErrNoRows { t.Skip("no product variants seeded; skipping") }
	...
	if err != nil { t.Fatalf("create from cart: %v", err) }
	if o.ID == "" { t.Fatalf("missing order id") }
	if o.Status != "pending_payment" { t.Fatalf("unexpected status: %s", o.Status) }
	if o.SubtotalCents <= 0 || o.TotalCents != o.SubtotalCents { t.Fatalf("totals invalid: %d %d", o.SubtotalCents, o.TotalCents) }
	if len(o.Items) == 0 { t.Fatalf("no order items") }
}
